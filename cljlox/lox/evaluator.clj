...
(defmethod evaluate :literal [node] (:value node))

(defmethod evaluate :grouping [node] (evaluate (:expression node)))
...
(defmethod execute :expr [node] (evaluate (:expression node)) nil)

(defn interpret
  [statements]
  ;; TODO: replace this with something more idiomatic like loop or reduce
  (doseq [stmt statements] (execute stmt)))
